...
- **타사 교육과정 표(`news/타사교육과정.csv`)는 카톡으로 안 보낸다.** 사용자가 "타사 교육 리스트 줘" 할 때만 꺼내 보여주는 자료다 — 예약 작업이 매주 2~4건씩 CSV에 쌓아두기만 하고, 발송 파이프라인에는 넣지 않는다 (사용자 결정). 표는 `python3 news/build_table.py`(HTML) / `--md`(채팅용 마크다운)로 뽑는다. **HTML은 생성물이므로 직접 고치지 말 것**
